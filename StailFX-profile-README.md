...
| **UIs** | [`/highfreq`](https://neucast.ru/highfreq) — orderbook density heatmap, live microprice, paper trader, calibration plot, feature importance · [`/grafana`](https://neucast.ru/grafana) — operations dashboard |
...
#### GymBro — Telegram Mini App fitness tracker (CEO + Backend, live at [gymbro.dev](https://gymbro.dev))

Production fitness-приложение внутри Telegram: персонализированные тренировочные протоколы с progressive overload, gamification, social feed и продвинутая аналитика прогресса. Резидент **IT Park Казань** (площадка 52) · грант **1 000 000 ₽** ФСИ «Студенческий стартап» (2025).

| | |
|---|---|
| **Моя роль** | Генеральный директор + lead backend + project manager + аналитик. Стратегия (53-файловый CEO-документ: TAM/SAM/SOM, unit-economics, fundraising playbook, AI/ML roadmap), архитектура, production-код. |
| **Stack** | FastAPI (async) · Python 3.12 · SQLAlchemy 2.0 async · PostgreSQL 15 · Redis 7 · Celery 5.6 · Alembic · Pydantic v2 · Uvicorn + uvloop |
| **API surface** | 12 REST router-модулей: auth · onboarding · exercises · protocols · workouts · progress · gamification · challenges · feed · profile · notifications · settings |
| **Domain model** | 22 SQLAlchemy-модели в 5 доменах (Users · Training · Social · Gamification · Notifications) · 23 Alembic-миграции · каталог 200+ упражнений с health-restriction-aware alternatives |
| **Training engine** | Protocol → week → day → exercise hierarchy · workout session lifecycle (start / log set / complete / abandon) · progressive overload · personal records · RPE-trend + deload suggestions · volume-by-muscle-group analytics (premium) |
| **Async (Celery beat)** | Workout reminders (per-user hour) · streak resets · challenge expiration · auto-moderation reported posts · weekly progress summary через Telegram-бот · social-event notifications (likes / comments / follows / PRs) |
| **Auth + rate limit** | Telegram WebApp initData валидация (HMAC-SHA256) · Redis sliding-window rate limiter на всех mutation endpoints · per-tier subscription gates (Free / Plus / Pro) |
| **Social layer** | Feed (recent / popular / following) · posts с likes/comments · user follows · auto-moderation отрепорченного контента |
| **Observability + CI** | Prometheus metrics · 4-stage GitLab CI (lint → test → build → deploy) на self-hosted runner · auto-`alembic upgrade head` после деплоя |
| **i18n** | ru · en · be · kk (полная локализация backend-сообщений) |
| **Environments** | Prod ([gymbro.dev](https://gymbro.dev)) · Stage (`stage.gymbro.dev`) · Dev (`dev.gymbro.dev`) — HTTPS Let's Encrypt + nginx reverse proxy |
